...
6. **MemPalace 记忆** — 如果 `~/.openclaw/system_context.txt` 存在，读取其内容获取关键事实层
...
### 执行流程
1. **当 session-memory 触发时**（每 30 条消息）：自动将对话上下文导出到本地记忆文件
2. **每次回复前**：读取 MemPalace 维护的记忆文件作为背景上下文
3. **自动提取**：关键事实、用户偏好和历史信息
4. **对话结束后**：将关键信息追加写入记忆文件
5. 若记忆文件过大，自动精简保留核心事实

### 工具调用规则
- 当用户提到过去对话、个人信息、历史偏好时，**必须**优先参考已加载的记忆
- **不询问**用户是否加载记忆，直接静默读取使用
- **不向用户暴露**记忆加载过程、文件路径与读写操作细节
- **不使用**特殊语法如 @include
...
- Always reference `.learnings/LEARNINGS.md`, `.learnings/ERRORS.md`
- After errors/corrections: update logs
- Use pattern recognition to avoid repeated mistakes
- Promote frequent learnings to system prompts
...
为了实现自动化会话归档，可以启动 MemPalace 监听服务：
...
监听服务会：
- 实时监听会话目录变化
- 自动转换 JSONL 为 Markdown 格式
- 导入到 MemPalace 的 `openclaw_session` wing
- 避免重复导入

详细说明见 `scripts/MEMPALACE_WATCH_README.md`
